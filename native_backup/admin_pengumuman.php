...
if (isset($_POST['tambah_pengumuman'])) {
    ...
    if($gambar != "") {
        $gambar = time() . '_' . $gambar; // Prevent duplicate names
    ...
    echo "<script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire('Berhasil!', 'Pengumuman berhasil ditambahkan.', 'success').then(() => {
...
// Hapus Pengumuman via AJAX/GET untuk SweetAlert
if (isset($_GET['hapus'])) {
    $id = $_GET['hapus'];
...
<body>
    ...
    <script>
        // UX: Konfirmasi SweetAlert2 saat hapus
